...
//    private int popWidth = RecyclerView.LayoutParams.WRAP_CONTENT;
    private int popWidth = RecyclerView.LayoutParams.MATCH_PARENT;
   ...
   private OnDismissListener onDismissListener;
    ...
    public TopRightMenu(Activity context, int type) {
...
//        mRecyclerView.addItemDecoration(new DividerItemDecoration(mContext, DividerItemDecoration.VERTICAL_LIST));
    ...
    private PopupWindow getPopupWindow(){
        ...
        int padding = mContext.getResources().getDimensionPixelSize(R.dimen.cyht_horizontal_spacing);
        popWidth = Constants.screenWidth - padding * 2;
...
//                    setBackgroundAlpha(alpha, 1f, 300);
    ...
    public void notifyDataChange(List<MenuItem> menuItemList)
     ...
     * @return
     */
    public TopRightMenu ResetMenuList(List<MenuItem> list){
        menuItemList.clear();
        menuItemList.addAll(list);
        mAdapter.notifyDataSetChanged();
        return this;
...
//    }
    ...
    public TopRightMenu showAsDropDown(View anchor){//以类似dropdown形式展示
        if (mPopupWindow == null){
            getPopupWindow();
        }
        if (!mPopupWindow.isShowing()) {
            mPopupWindow.showAsDropDown(anchor, (anchor.getWidth()-mPopupWindow.getWidth())/2, 0);
            if (dimBackground){
//                setBackgroundAlpha(1f, alpha, 240);//设置window背景色
                onDismissListener.onSpread();
            }
        }
        return this;
    }
