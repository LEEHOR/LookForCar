package com.cyht.wykc.widget.ClipImage;
...
public class ClipViewLayout extends RelativeLayout {
    ...
    public void initSrcPic(Uri uri) {
        ...
        String path = FileUtils.getRealFilePathFromUri(getContext(), uri);
        ...
        Bitmap bitmap = ImageUtil.scaleImageFromAbsolutePath(path, 720, 1280);
        if (bitmap == null) {
            return;
        }

        //竖屏拍照的照片，直接使用的话，会旋转90度，下面代码把角度旋转过来
        int rotation = ImageUtil.getExifOrientation(path); //查询旋转角度
...
//            zoomedCropBitmap = ImageUtil.scaleBitmapTo(cropBitmap, 200, 200);
        } catch (Exception e) {
            e.printStackTrace();
        }
//        if (cropBitmap != null) {
